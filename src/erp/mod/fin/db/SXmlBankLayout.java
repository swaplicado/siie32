...
public class SXmlBankLayout extends SXmlDocument {
    ...
    public void processXmlMail(String xml) throws Exception {
        clear();
        ...
        for (Node paymentNode : paymentNodes) {
            ...
            boolean found = false;
            for (SXmlBankLayoutPayment xmlPaymentAux : xmlPaymentsAux) { 
                found = false;
                if (SLibUtilities.compareKeys(new int[] { SLibUtils.parseInt(SXmlUtils.extractAttributeValue(paymentNodeMap, SXmlBankLayoutPayment.ATT_LAY_PAY_BANK_BPB, true)), SLibUtils.parseInt(SXmlUtils.extractAttributeValue(paymentNodeMap, SXmlBankLayoutPayment.ATT_LAY_PAY_BANK_BANK, true)),  SLibUtils.parseInt(SXmlUtils.extractAttributeValue(paymentNodeMap, SXmlBankLayoutPayment.ATT_LAY_PAY_BANK_KEY, true)) },
                        new int[] { (int) xmlPaymentAux.getAttribute(SXmlBankLayoutPayment.ATT_LAY_PAY_BANK_BPB).getValue(), (int) xmlPaymentAux.getAttribute(SXmlBankLayoutPayment.ATT_LAY_PAY_BANK_BANK).getValue(), (int) xmlPaymentAux.getAttribute(SXmlBankLayoutPayment.ATT_LAY_PAY_BANK_KEY).getValue() })) {
                    found = true;
                }
                if (found) {
                    xmlPaymentAux.getAttribute(SXmlBankLayoutPayment.ATT_LAY_PAY_AMT).setValue((double) xmlPaymentAux.getAttribute(SXmlBankLayoutPayment.ATT_LAY_PAY_AMT).getValue() + SLibUtils.parseDouble(SXmlUtils.extractAttributeValue(paymentNodeMap, SXmlBankLayoutPayment.ATT_LAY_PAY_AMT, true)));
                    xmlPaymentAux.getAttribute(SXmlBankLayoutPayment.ATT_LAY_PAY_AMT_CY).setValue((double) xmlPaymentAux.getAttribute(SXmlBankLayoutPayment.ATT_LAY_PAY_AMT_CY).getValue() + SLibUtils.parseDouble(SXmlUtils.extractAttributeValue(paymentNodeMap, SXmlBankLayoutPayment.ATT_LAY_PAY_AMT_CY, true)));
                    
                    //al que se agrega
                    xmlPaymentAux.getAttribute(SXmlBankLayoutPayment.ATT_LAY_PAY_REF_ALPHA).getValue();
                    //el que se agrega
                    SXmlUtils.extractAttributeValue(paymentNodeMap, SXmlBankLayoutPayment.ATT_LAY_PAY_REF_ALPHA, true);
            ...
            xmlPaymentsAux.add(xmlPayment);
            }
        }
        xmlPaymentsAux.stream().forEach((xmlPaymentAux) -> {
            mvXmlElements.add(xmlPaymentAux);
        });
    }
}
