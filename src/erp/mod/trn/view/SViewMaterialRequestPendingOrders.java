...
public class SViewMaterialRequestPendingOrders extends SGridPaneView implements ActionListener{
    ...
    private void initComponents() {
        ...
        mjbToEstimate = SGridUtils.createButton(new ImageIcon(getClass().getResource("/erp/img/icon_std_money_out.gif")), "Solicitar cotización", this);
        ...
        if (jtTable.getSelectedRowCount() != 1) {
            ...
            if (gridRow.getRowType() != SGridConsts.ROW_TYPE_DATA) {
                ...
                try {
                    ...
                    if (miClient.showMsgBoxConfirm("¿Está seguro de regresar al solicitante?\n"
    ...
    private void actionToSupply() {
        if (jtTable.getSelectedRowCount() != 1) {
            ...
            if (gridRow.getRowType() != SGridConsts.ROW_TYPE_DATA) {
                ...
                if (miClient.showMsgBoxConfirm("¿Está seguro de regresar la requisición a suministro?") == JOptionPane.OK_OPTION) {
                    int[] key = (int[]) gridRow.getRowPrimaryKey();
        ...
        if (jtTable.getSelectedRowCount() != 1) {
            ...
            if (gridRow.getRowType() != SGridConsts.ROW_TYPE_DATA) {
                ...
                try {
                    ...
                    else if (moMaterialRequest.isCloseQuotation()) {
                        miClient.showMsgBoxWarning("La requisición no puede cotizarse porque está 'CERRADA PARA COTIZAR'");
                        return;
                    }
                    
                    moDialogEstimate = new SDialogMaterialRequestEstimation(miClient, "Solicitar cotización de requisición");
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT v.id_mat_req AS " + SDbConsts.FIELD_ID + "1, "
                ...
                + "ve.id_mat_req = mre.fk_mat_req_n AND ve.id_ety = mre.fk_mat_req_ety_n AND NOT mre.b_del "
