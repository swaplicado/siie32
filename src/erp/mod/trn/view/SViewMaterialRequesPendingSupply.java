 ...
 */
public class SViewMaterialRequesPendingSupply extends SGridPaneView implements ActionListener {
     ...
     * @param params
     */
    public SViewMaterialRequesPendingSupply(SGuiClient client, int type, int subtype, String title, SGuiParams params) {
...
//        mjbOpen.setSize(23, 23);
        ...
        mjbToNew.setEnabled(mnGridSubtype == SLibConstants.UNDEFINED);
        mjbToPur.setEnabled(mnGridSubtype == SLibConstants.UNDEFINED);
...
//        getPanelCommandsSys(SGuiConsts.PANEL_CENTER).add(mjbEstimationKardex);
        ...
        if (mnGridType == SModConsts.TRNX_MAT_REQ_STK_SUP && (mnGridSubtype == SModSysConsts.TRNX_MAT_REQ_PROVIDED || mnGridSubtype == SModSysConsts.TRNX_MAT_REQ_PROVIDED_DETAIL)){
        ...
        if (mnGridType == SModConsts.TRNX_MAT_REQ_STK_SUP) {
            if (mnGridSubtype == SModSysConsts.TRNX_MAT_REQ_DETAIL) {
                select = "i.item, i.item_key, u.unit, ve.id_ety, "
                        ...
                        + "COALESCE(SUM(ve.qty) - SUM(de.sumi_qty), SUM(ve.qty)) AS pen_sumi_qty, "
                        ...
                        + " WHERE mrl.id_mat_req = v.id_mat_req AND mrl.fk_st_mat_req = " + SModSysConsts.TRNS_ST_MAT_REQ_PROV + ") = 1, " + SGridConsts.ICON_CIRC_MAGENTA + ", " + SGridConsts.ICON_CIRC_WHITE + ") "
                        + " AS count_sum, ";
                ...
                having = "HAVING org_qty > sumi_qty "; // Descomentar para mostrar unicamente los que faltan por suministrar
            }
                ...
                select = "COUNT(ve.id_ety) AS ety, " 
                        ...
                        + "IF (1 - COALESCE(SUM(de.sumi_qty), 0) / SUM(ve.qty) < 1, 0, "
                ...
                subGroupOrderBy = "de.fid_mat_req_n, de.fid_mat_req_ety_n ";
                having = "HAVING SUM(de.sumi_qty) >= SUM(ve.qty) OR (v.b_clo_prov AND v.fk_st_mat_req = " + SModSysConsts.TRNS_ST_MAT_REQ_PROV + ") ";
                if (moFiltersMap.get(SGridConsts.FILTER_DATE_PERIOD) != null) {
                    filter = (SGuiDate) moFiltersMap.get(SGridConsts.FILTER_DATE_PERIOD).getValue();
                    if (filter != null) {
                        where += (where.isEmpty() ? "" : "AND ") + SGridUtils.getSqlFilterDate("v.dt", (SGuiDate) filter);
                ...
                subGroupOrderBy = "de.fid_mat_req_n, de.fid_mat_req_ety_n ";
                where += (where.isEmpty() ? "" : "AND ") + " v.tp_req = '" + SModSysConsts.TRNS_MAT_REQ_TP_R + "' ";
                having = "HAVING sumi_qty >= org_qty OR (v.b_clo_prov AND v.fk_st_mat_req = " + SModSysConsts.TRNS_ST_MAT_REQ_PROV + ") ";
                if (moFiltersMap.get(SGridConsts.FILTER_DATE_PERIOD) != null) {
                    filter = (SGuiDate) moFiltersMap.get(SGridConsts.FILTER_DATE_PERIOD).getValue();
                    if (filter != null) {
                        where += (where.isEmpty() ? "" : "AND ") + SGridUtils.getSqlFilterDate("v.dt", (SGuiDate) filter);
                ...
                groupOrderBy = "v.id_mat_req, v.dt, v.num ";
                subGroupOrderBy = "de.fid_mat_req_n, de.fid_mat_req_ety_n ";
                having = "HAVING COALESCE(SUM(ve.qty), 0) > COALESCE(SUM(de.sumi_qty), 0) "; // cantidad de la requisición > cantidad suministrada
            }
        }
    ...
    public void defineSuscriptions() {
        ...
        moSuscriptionsSet.add(SModConsts.TRNX_MAT_REQ_STK_SUP);
    }
