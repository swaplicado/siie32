 ...
 */
public class SViewComissionsFromRecords extends SGridPaneView implements ActionListener {
     ...
     */
    public SViewComissionsFromRecords(SGuiClient client, String title) {
        super(client, SGridConsts.GRID_PANE_VIEW, SModConsts.MKT_COMMS_PAY_REC, SLibConsts.UNDEFINED, title);
    ...
    public void prepareSqlQuery() {
        ...
        if (filter != null && ((int[]) filter).length == 1) {
            filterSql += "AND IF(cb.fid_sal_agt_n > 0, cb.fid_sal_agt_n, ccus.fid_sal_agt_n) = " + ((int[]) filter)[0] + " ";
        }
        
        moPaneSettings = new SGridPaneSettings(1);
        
        msSql = "SELECT " +
            "ety.id_ety AS " + SDbConsts.FIELD_ID + "1, " +
            "ety.id_num AS " + SDbConsts.FIELD_ID + "2, " +
            "ety.id_tp_rec AS " + SDbConsts.FIELD_CODE + ", " +
            "ety.id_tp_rec AS " + SDbConsts.FIELD_NAME + ", " +
            "rec.dt AS " + SDbConsts.FIELD_DATE + ", " +
            "ety.concept, bp.bp, bp.id_bp, " +
            "CONCAT(rec.id_year, '-', erp.lib_fix_int(rec.id_per, 2)) AS f_rper, CONCAT(rec.id_tp_rec, '-', erp.lib_fix_int(rec.id_num, 6)) AS f_rnum, " +
            "ety.debit, " +
            "bpa.id_bp AS f_id_sal_agt, bpa.bp AS f_sal_agt, " +
            "COALESCE(IF((SELECT agt_per FROM mkt_comms_sal_agt WHERE id_tp_link = " + SModSysConsts.ITMS_LINK_ALL + " AND " +
            "id_sal_agt = IF(cb.fid_sal_agt_n > 0, cb.fid_sal_agt_n, ccus.fid_sal_agt_n)) IS NOT NULL, " +
            "(SELECT agt_per FROM mkt_comms_sal_agt WHERE id_tp_link = " + SModSysConsts.ITMS_LINK_ALL + " AND id_sal_agt = IF(cb.fid_sal_agt_n > 0, cb.fid_sal_agt_n, ccus.fid_sal_agt_n)), " +
            ...
            "WHERE id_tp_link = " + SModSysConsts.ITMS_LINK_ALL + " AND id_sal_agt = IF(cb.fid_sal_agt_n > 0, cb.fid_sal_agt_n, ccus.fid_sal_agt_n))), 0) * ety.debit AS agt_perc " +
            "FROM fin_rec AS rec " +
            ...
            "LEFT OUTER JOIN erp.bpsu_bp AS bpa ON IF(cb.fid_sal_agt_n > 0, cb.fid_sal_agt_n, ccus.fid_sal_agt_n) = bpa.id_bp " +
            ...
            filterSql +
            "ORDER BY bp.bp, bp.id_bp, f_date, f_sal_agt, agt_perc; ";
    }
    ...
    public ArrayList<SGridColumnView> createGridColumns() {
        ...
        columns.add(new SGridColumnView(SGridConsts.COL_TYPE_TEXT, "f_sal_agt", "Agente ventas"));
        columns.add(new SGridColumnView(SGridConsts.COL_TYPE_DEC_2D, "agt_perc", "Comisión"));
