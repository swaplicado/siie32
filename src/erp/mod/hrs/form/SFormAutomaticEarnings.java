...
public class SFormAutomaticEarnings extends SBeanForm implements SGridPaneFormOwner, ActionListener, ItemListener {
    ...
    private void initComponents() {
        ...
        moTextCode.setToolTipText("Percepción");
        ...
        jbPick.setText("...");
        jbPick.setToolTipText("Seleccionar percepción");
    ...
    private void initComponentsCustom() {
        ...
        switch (mnFormSubtype) {
            case SModSysConsts.HRS_AUT_EMP:
                jlWarning.setText("ADVERTENCIA: ¡Las percepciones configuradas a nivel de empleado reemplazarán a las percepciones configuradas a nivel global!");
                break;
            default:
                jlWarning.setText("");
        }
    ...
    private void resetLoan() {
        if (mnFormSubtype == SModSysConsts.HRS_AUT_EMP && moKeyEmployee.getSelectedIndex() > 0 && moEarning != null && moEarning.isLoan()) {
    ...
    private void updateRows() {
        ...
        automaticRow.setUnits(moEarning.isBasedOnUnits() ? moComValue.getField().getValue() : 1);
        automaticRow.setAmountUnitary(!moEarning.isBasedOnUnits() ? moComValue.getField().getValue() : 0); // when necessary, amount unit will be provided by system
        ...
        if (moKeyLoan_n.isEnabled() && moKeyLoan_n.getSelectedIndex() > 0) {
    ...
    private void actionPick() {
        miClient.getSession().showOptionPicker(SModConsts.HRS_EAR, SLibConsts.UNDEFINED, null, moTextCode);
        actionCode();
    }

    private void actionCode() {
        try {
            moEarning = SHrsUtils.getEarning(miClient, SLibUtils.textTrim(moTextCode.getText()));
        }
        catch (Exception e) {
        ...
        if (moEarning == null) {
            miClient.showMsgBoxWarning("No se encontró ninguna percepción con el código '" + moTextCode.getText() + "'.");
            moTextCode.requestFocusInWindow();
        }
        else {
            if (moEarning.isBenefit()) {
                miClient.showMsgBoxWarning("No se pueden agregar percepciones automáticas de tipo prestación.");
                moTextCode.requestFocusInWindow();
            }
            else if (moEarning.isAbsence()) {
                miClient.showMsgBoxWarning("No se pueden agregar percepciones automáticas de tipo incidencia.");
                moTextCode.requestFocusInWindow();
            }
            else if (moEarning.isLoan() && mnFormSubtype == SModSysConsts.HRS_AUT_GBL) {
                miClient.showMsgBoxWarning("No se pueden agregar percepciones automáticas de tipo crédito/préstamo de forma global.");
                moTextCode.requestFocusInWindow();
            }
            else {
                ...
                if (moEarning.isComputedByPercentage()) {
                    moComValue.getField().setValue(moEarning.getPayPercentage());
                }
    ...
    private void actionAdd() {
        ...
        else if (mnFormSubtype == SModSysConsts.HRS_AUT_GBL && moEarning.isLoan()) {
        ...
        else if (mnFormSubtype == SModSysConsts.HRS_AUT_EMP && moEarning.isLoan() && moKeyLoan_n.getSelectedIndex() <= 0) {
