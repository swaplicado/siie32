...
public class SDbCfgAccountingDeduction extends SDbRegistryUser {
    ...
    */
        ...
        if (can) {
            ...
            if (isAccountingRecordTypeGlobal) {
                if (mnFkAccountId == SDataConstantsSys.NA) {
                    throw new Exception("Se debe especificar una cuenta contable, porque el tipo de registro contable de la deducción es 'global'.");
                }
            }
            else {
    ...
    public static final int countExistingRegistries(final SGuiSession session, final int idToCount) throws Exception {
        int count = 0;
        String sql = "SELECT COUNT(*) FROM " + SModConsts.TablesMap.get(SModConsts.HRS_CFG_ACC_DED) + " WHERE id_ded = " + idToCount + ";";
