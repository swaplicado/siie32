...
public class SViewAccountingEarning extends SGridPaneView implements ActionListener {
    ...
    private SGridFilterPanel moFilterEarning;
    ...
    public void prepareSqlQuery() {
        ...
        filter = ((SGridFilterValue) moFiltersMap.get(SModConsts.HRS_EAR)).getValue();
        if (filter != null && ((int[]) filter).length == 1) {
            sql += (sql.isEmpty() ? "" : "AND ") + "v.id_ear = " + ((int[]) filter)[0] + " ";
        }
        ...
        msSql = "SELECT "
                ...
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.HRS_EAR) + " AS ear ON v.id_ear = ear.id_ear "
        ...
        msSql += "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.FIN_ACC) + " AS ac ON v.fk_acc = ac.pk_acc "
                ...
                + "ORDER BY ear.code, ear.name, f_ref, v.id_ear, v.id_tp_acc ";
    }
    ...
    public ArrayList<SGridColumnView> createGridColumns() {
        ...
        gridColumnsViews.add(new SGridColumnView(SGridConsts.COL_TYPE_TEXT_CODE_CAT, SDbConsts.FIELD_CODE, "Código percepción"));
