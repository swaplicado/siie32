 ...
 * @author Juan Barajas
 */
public class SViewEmployeeType extends SGridPaneView {

    public SViewEmployeeType(SGuiClient client, String title) {
        super(client, SGridConsts.GRID_PANE_VIEW, SModConsts.HRSU_TP_EMP, SLibConsts.UNDEFINED, title);
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT "
                + "v.id_tp_emp AS " + SDbConsts.FIELD_ID + "1, "
                ...
                + "FROM " + SModConsts.TablesMap.get(SModConsts.HRSU_TP_EMP) + " AS v "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.USRU_USR) + " AS ui ON "
                + "v.fk_usr_ins = ui.id_usr "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.USRU_USR) + " AS uu ON "
                + "v.fk_usr_upd = uu.id_usr "
                + (sql.isEmpty() ? "" : "WHERE " + sql)
                + "ORDER BY v.name, v.id_tp_emp ";
    }
