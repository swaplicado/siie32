    ...
    public void prepareSqlQuery() {
        ...
        */
        ...
        msSql = "SELECT "
                ...
                + " "
                // # cut off date:
                + "@cut_off := IF(e.b_act, " + pDateCutOff + ", e.dt_dis_n) AS _p_dt_cutoff, "
                + " "
                ...
                */
                ...
                + "    WHERE pre.fk_tp_ben = " + pBenefitType + " " + (where.isEmpty() ? "" : " AND " + where) + "AND NOT p.b_del AND NOT pr.b_del AND NOT pre.b_del AND pre.ben_year = YEAR(@cut_off) "
                ...
                + "    INNER JOIN " + SModConsts.TablesMap.get(SModConsts.HRSU_EMP) + " AS e ON pr.id_emp = e.id_emp "
                + "    WHERE prd.fk_tp_ben = " + pBenefitType + " " + (where.isEmpty() ? "" : " AND " + where) + "AND NOT p.b_del AND NOT pr.b_del AND NOT prd.b_del AND prd.ben_year = YEAR(@cut_off) "
                ...
                + "    INNER JOIN " + SModConsts.TablesMap.get(SModConsts.HRS_PAY_RCP_EAR) + " AS pre ON pre.id_pay = pr.id_pay AND pre.id_emp = pr.id_emp "
                + "    INNER JOIN " + SModConsts.TablesMap.get(SModConsts.HRSU_EMP) + " AS e ON pr.id_emp = e.id_emp "
                + "    WHERE pre.fk_tp_ben = " + pBenefitType + " " + (where.isEmpty() ? "" : " AND " + where) + "AND NOT p.b_del AND NOT pr.b_del AND NOT pre.b_del AND pre.ben_year = YEAR(@cut_off) - 1 "
                + "    GROUP BY pre.id_emp, pre.ben_year, pre.ben_ann "
                + "    UNION "
                //     # benefit refund to employer as deduction:
                + "    SELECT prd.id_emp, prd.ben_year, prd.ben_ann, -SUM(prd.unt) AS ben_unt, -SUM(prd.amt_r) AS ben_amt "
                ...
                + "    INNER JOIN " + SModConsts.TablesMap.get(SModConsts.HRS_PAY_RCP_DED) + " AS prd ON prd.id_pay = pr.id_pay AND prd.id_emp = pr.id_emp "
                + "    INNER JOIN " + SModConsts.TablesMap.get(SModConsts.HRSU_EMP) + " AS e ON pr.id_emp = e.id_emp "
                + "    WHERE prd.fk_tp_ben = " + pBenefitType + " " + (where.isEmpty() ? "" : " AND " + where) + "AND NOT p.b_del AND NOT pr.b_del AND NOT prd.b_del AND prd.ben_year = YEAR(@cut_off) - 1 "
                + "    GROUP BY prd.id_emp, prd.ben_year, prd.ben_ann "
                + "    ORDER BY id_emp, ben_year, ben_ann) AS t "
                + "  GROUP BY id_emp, ben_year, ben_ann "
                + "  ORDER BY id_emp, ben_year, ben_ann) AS tprev ON tprev.id_emp = bp.id_bp "
                + " "
                + "WHERE e.dt_ben <= " + pDateCutOff + " AND (e.b_act OR (NOT e.b_act AND e.dt_dis_n <= " + pDateCutOff + ")) " + (sql.isEmpty() ? "" : "AND " + sql)
                + " "
