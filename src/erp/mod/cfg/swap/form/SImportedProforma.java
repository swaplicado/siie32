...
public class SImportedProforma implements SGridRow, Serializable, Comparable<SImportedProforma> {
    ...
    public SImportedProforma(final JsonNode docNode, final int externalId, final int funcSubAreaId,
            final boolean alreadyDownloaded, final SGuiSession session,
            final PreparedStatement prepStatProcessing) throws Exception {
        this();

        ExternalDocumentId = externalId;
        ExternalDocumentUuid = docNode.has("uuid") && !docNode.path("uuid").isNull() ? docNode.path("uuid").asText() : "";

        retrieveProcessing(session, prepStatProcessing, SDbSwapDataProcessing.DATA_TYPE_PRF, SDataConstantsSys.TRNS_CT_DPS_PUR, ExternalDocumentId);
        ...
        if (NumberSeries.isEmpty() && Number.isEmpty() && ExternalDocumentUuid.isEmpty()) {
        ...
        DueDate = dueDateAsText.isEmpty() || dueDateAsText.equals("null") ? null : SLibUtils.IsoFormatDate.parse(dueDateAsText);

        JsonNode referencesNode = docNode.path("references");
        if (referencesNode.isArray()) {
            java.util.ArrayList<Reference> references = new java.util.ArrayList<>();
            for (JsonNode referenceNode : referencesNode) {
                references.add(new Reference(
                        referenceNode.get("document_ref_type").asInt(),
                        referenceNode.get("reference").asText(),
                        SImportUtils.createDpsKey(referenceNode.get("external_id").asText())));
            }
            if (!references.isEmpty()) {
                References = references.toArray(new Reference[0]);
                ReferencesType = references.get(0).ReferenceType;
                ReferencesAsText = composeReferences();
            }
        }

        Description = docNode.get("notes").asText();
        ...
        int reqPayDef = docNode.has("payment_definition") ? docNode.get("payment_definition").asInt() : PAY_NOT_REQ;
        double reqPayAmt = docNode.has("payment_amount") ? SLibUtils.parseDouble(docNode.get("payment_amount").asText()) : 0d;
        double reqPayPct = SLibUtils.parseDouble(docNode.get("payment_percentage").asText());
        String reqPayDateText = docNode.has("payment_date") && !docNode.path("payment_date").isNull() ? docNode.get("payment_date").asText() : "";
        Date reqPayDate = reqPayDateText.isEmpty() || reqPayDateText.equals("null") ? null : SLibUtils.IsoFormatDate.parse(reqPayDateText);

        if (reqPayDate == null && reqPayPct == 0) {
            RequiredPaymentDefinition = PAY_NOT_REQ;
            RequiredPaymentNotes = docNode.get("payment_notes").asText();
        }
        else {
            RequiredPaymentDefinition = reqPayDef != PAY_NOT_REQ ? reqPayDef : (reqPayPct > 0 ? PAY_DEF_BY_PCT : PAY_DEF_BY_AMT);
            RequiredPaymentAmount = reqPayAmt;
            RequiredPaymentPct = reqPayPct;
            RequiredPaymentDate = reqPayDate;
            IsRequiredPaymentLoc = docNode.get("is_payment_loc").asBoolean();
            RequiredPaymentNotes = docNode.get("payment_notes").asText();
        }

        String revDateText = docNode.has("date_week_revision") ? docNode.get("date_week_revision").asText() : docNode.get("authorized_at").asText();
        RevisionDatetime = docNode.path("date_week_revision").isNull() || revDateText == null || revDateText.isEmpty() || revDateText.equals("null") ? null
                : SSwapUtils.SwapDatetimeMicrosecsTimeZoneFormat.parse(revDateText.replaceFirst("(\\.\\d{3})\\d+", "$1"));
    ...
    /**
     * Ger required payment amount of document, directly defined or indirectly
     * from required payment percentage.
     *
     * @return
     */
     ...
     * @param dataType Constants DATA_TYPE...: INV = invoice; DB = debit note;
     * CN = credit note.
    ...
    /**
     * Get Processed Proforma from SWAP processed data, if any, by its external
     * ID.
     *
     * @param preparedStatement Prepared statement.
     * @param dataType Constants DATA_TYPE...: INV = invoice; DB = debit note;
     * CN = credit note; PRF = proforma.
     * @param txnCategory Transaction category: 1 = purchase; 2 = sales.
     * @param externalId External ID.
    ...
    /**
     * Create prepared statement to get Processed DPS from Payable or Receivable
     * Accounts by its own document data.
     *
     * @param statement DB statement.
     * @param dpsTypeKey Key of DPS type: (category, class & type).
     * @return A prepared statment with these columns: dps_id_year, dps_id_doc,
     * rec_id_year, rec_id_per, rec_id_bkc, rec_id_tp_rec, rec_id_num,
     * rec_cob_code.
     * @throws Exception
     */
    ...
    /**
     * Get DPS primary key from Payable or Receivable Accounts, if any, by its
     * own document data.
     *
     * @param preparedStatement Prepared statement.
