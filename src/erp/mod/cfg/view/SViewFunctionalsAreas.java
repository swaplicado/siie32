...
public class SViewFunctionalsAreas extends SGridPaneView {
    ...
    public void prepareSqlQuery() {
        ...
        msSql = "SELECT "
                + "v.id_func AS " + SDbConsts.FIELD_ID + "1, "
                ...
                + "FROM " + SModConsts.TablesMap.get(SModConsts.CFGU_FUNC) + " AS v "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.USRU_USR) + " AS ui ON "
                + "v.fk_usr_ins = ui.id_usr "
                + "INNER JOIN " + SModConsts.TablesMap.get(SModConsts.USRU_USR) + " AS uu ON "
                + "v.fk_usr_upd = uu.id_usr "
                + (sql.isEmpty() ? "" : "WHERE " + sql)
                + "ORDER BY v.name, v.id_func ";
    }
    ...
    public ArrayList<SGridColumnView> createGridColumns() {
        ...
        gridColumnsViews.add(new SGridColumnView(SGridConsts.COL_TYPE_DEC_AMT, "v.exp_mon", "Límite mensual $"));
