...
public class SViewDpsStockSupply extends erp.lib.table.STableTab implements java.awt.event.ActionListener {
    ...
    private javax.swing.JButton mjbSupply;
     ...
     * @param tabType View tab type. Constants defined in SDataConstats (TRNX_DPS_SUPPLY_PEND, TRNX_DPS_SUPPLY_PEND_ETY, TRNX_DPS_SUPPLIED, TRNX_DPS_SUPPLIED_ETY).
    ...
    private boolean isViewForDocuments() {
        return mnTabType == SDataConstants.TRNX_DPS_SUPPLY_PEND || mnTabType == SDataConstants.TRNX_DPS_SUPPLIED;
    }

    private boolean isViewForDocumentEntries() {
        return mnTabType == SDataConstants.TRNX_DPS_SUPPLY_PEND_ETY || mnTabType == SDataConstants.TRNX_DPS_SUPPLIED_ETY;
    }
    ...
    private boolean isViewForSupply() {
        return mnTabType == SDataConstants.TRNX_DPS_SUPPLY_PEND || mnTabType == SDataConstants.TRNX_DPS_SUPPLY_PEND_ETY;
    }
    ...
    private void initComponents() {
        ...
        mjbSupply = new JButton(isViewForPurchases() ?
        ...
        mjbSupply.setPreferredSize(new Dimension(23, 23));
        ...
        mjbSupply.addActionListener(this);
        ...
        mjbSupply.setToolTipText("Surtir");
        ...
        if (isViewForPurchases()) {
            ...
            levelRightDocTransaction = miClient.getSessionXXX().getUser().hasRight(miClient, SDataConstantsSys.PRV_INV_IN_PUR).Level;
        }
        else {
            levelRightAllDocs = miClient.getSessionXXX().getUser().hasRight(miClient, SDataConstantsSys.PRV_SAL_DOC_TRN).Level;
            levelRightDocTransaction = miClient.getSessionXXX().getUser().hasRight(miClient, SDataConstantsSys.PRV_INV_OUT_SAL).Level;
        }
        ...
        addTaskBarUpperComponent(isViewForSupply() ? moTabFilterDateCutOff : moTabFilterDatePeriod);
        addTaskBarUpperSeparator();
        ...
        aoTableColumns[i].getRpnArguments().add(new SLibRpnArgument("f_adj_orig_qty", SLibRpnArgumentType.OPERAND));
        aoTableColumns[i].getRpnArguments().add(new SLibRpnArgument(SLibRpnOperator.SUBTRACTION, SLibRpnArgumentType.OPERATOR));
        aoTableColumns[i].getRpnArguments().add(new SLibRpnArgument("f_sup_orig_qty", SLibRpnArgumentType.OPERAND));
        aoTableColumns[i++].getRpnArguments().add(new SLibRpnArgument(SLibRpnOperator.SUBTRACTION, SLibRpnArgumentType.OPERATOR));
        if (isViewForDocumentEntries()) {
            aoTableColumns[i++] = new STableColumn(SLibConstants.DATA_TYPE_STRING, "f_orig_unit", "Unidad", STableConstants.WIDTH_UNIT_SYMBOL);
        }
