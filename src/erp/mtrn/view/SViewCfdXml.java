...
public class SViewCfdXml extends erp.lib.table.STableTab implements java.awt.event.ActionListener {
    ...
    public void createSqlQuery() {
        ...
        msSql += "INNER JOIN erp.bpsu_bpb AS bpb ON d.fid_bpb = bpb.id_bpb " +
                 ...
                 "WHERE dx.fid_tp_cfd = " + (isCfdiPayroll() ? SDataConstantsSys.TRNS_TP_CFD_PAYROLL : SDataConstantsSys.TRNS_TP_CFD_INV) + " AND dx.fid_tp_xml IN (" + SDataConstantsSys.TRNS_TP_XML_CFDI_32 + ", " + SDataConstantsSys.TRNS_TP_XML_CFDI_33 + ", " + SDataConstantsSys.TRNS_TP_XML_CFDI_40 + ") AND d.b_del = 0 " + 
         ...
         msSql += (isCfdiPayrollVersionOld() ? "INNER JOIN hrs_sie_pay_emp AS hr ON dx.fid_pay_pay_n = hr.id_pay AND dx.fid_pay_emp_n = hr.id_emp AND dx.fid_pay_bpr_n = hr.fid_bpr_n AND hr.b_del = FALSE " : 
                 ...
                 "INNER JOIN erp.trns_tp_cfd AS tp ON dx.fid_tp_cfd = tp.id_tp_cfd " +
                 "WHERE dx.fid_tp_cfd = " + (isCfdiPayroll() ? SDataConstantsSys.TRNS_TP_CFD_PAYROLL : SDataConstantsSys.TRNS_TP_CFD_INV)+ " AND dx.fid_tp_xml IN (" + SDataConstantsSys.TRNS_TP_XML_CFDI_32 + ", " + SDataConstantsSys.TRNS_TP_XML_CFDI_33 + ", " + SDataConstantsSys.TRNS_TP_XML_CFDI_40 + ") AND hr.b_del = 0 " +
                 "AND NOT (dx.fid_st_xml = " + SDataConstantsSys.TRNS_ST_DPS_NEW + " AND dx.b_con = 0) " + (isCfdiSignPending() ? " AND LENGTH(dx.uuid) = 0 " : " AND LENGTH(dx.uuid) <> 0 ") + 
                 sqlDatePeriodCfd +
