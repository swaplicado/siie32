    ...
    public void computeValue() {
        if (!SLibUtilities.belongsTo(getDiogClassKey(), new int[][] { SDataConstantsSys.TRNS_CL_IOG_IN_EXP, SDataConstantsSys.TRNS_CL_IOG_OUT_EXP })) {
            ...
            if (mnFkDiogCategoryId == SDataConstantsSys.TRNS_CT_IOG_IN) {
                if (mdCostUnitary != 0 && SLibUtilities.round(mdDebit, 2) != SLibUtilities.round(mdMoveIn * mdCostUnitary, 2)) {
                ...
                mdDebit = 0;
                if (mdCostUnitary != 0 && SLibUtilities.round(mdCredit, 2) != SLibUtilities.round(mdMoveOut * mdCostUnitary, 2)) {
