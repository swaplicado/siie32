 ...
 */
public class SDataLeadtimeCo extends erp.lib.data.SDataRegistry implements java.io.Serializable {
    ...
    protected java.util.Vector<erp.mmfg.data.SDataLeadtimeCo> mvDbmsTypes;

    protected int mnDbmsAuxSortingItem;

    public SDataLeadtimeCo() {
        super(SDataConstants.MFG_LT_COB);

        mvDbmsTypes = new Vector<SDataLeadtimeCo>();
        reset();
    }
    ...
    public java.util.Vector<SDataLeadtimeCo> getDbmsTypes() { return mvDbmsTypes; }

    private java.lang.String query(java.lang.Object params) {
        int[] key = (int[]) params;
        String sql = "";

        sql = "SELECT t.id_sup, t.id_ety, t.ltime, t.b_del, b.bp, " +
            "CASE t.fid_tp_link " +
            ...
            "WHERE t.id_sup = " + key[0] + (key[1] == 0 ? " " : " AND t.id_ety = " + key[1] + " ");
    ...
    public void setPrimaryKey(java.lang.Object key) {
        mnPkSupplierId = ((int[]) key)[0];
        mnPkEntryId = ((int[]) key)[1];
    }

    @Override
    public int[] getPrimaryKey() {
        return new int[] { mnPkSupplierId, mnPkEntryId };
    }
    ...
    public int read(java.lang.Object pk, java.sql.Statement statement) {
        ...
        try {
            ...
            if (!resultSet.next()) {
                ...
                sql = query (new int[] { key[0], 0 });
                resultSet = statement.executeQuery(sql);
                while (resultSet.next()) {
                    type = new SDataLeadtimeCo();
    ...
    public int save(java.sql.Connection connection) {
        ...
        try {
            ...
            for (int i = 0; i < mvDbmsTypes.size(); i++) {
                type = (SDataLeadtimeCo) mvDbmsTypes.get(i);
                nParam = 1;

                if (type != null) {
                    callableStatement = connection.prepareCall("{" +
                    " CALL mfg_ltime_co_save(" +
                        "?, ?, ?, ?, ?, " +
                        "?, ?, ?, ?, ? " +
                        ") }");
