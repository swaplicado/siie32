 ...
 */
public class SFormDrcEntry extends javax.swing.JDialog implements erp.lib.form.SFormInterface, java.awt.event.ActionListener {
    ...
    public SFormDrcEntry(erp.client.SClientInterface client) {
        super(client.getFrame(), true);
        miClient = client;
        mnFormType = SDataConstants.MFG_DRC_ETY;
    ...
    public void formRefreshCatalogues() {
        ...
        if (mbDirectCost) {
            SFormUtilities.populateComboBox(miClient, jcbFkEntityId, SDataConstants.MFG_ORD, new Object[] { "" +
                    SDataConstantsSys.MFGS_ST_ORD_NEW + ", " +
                    SDataConstantsSys.MFGS_ST_ORD_PROC, SDataConstants.MFGX_ORD_MAIN_NA, false });
        }
        else {
            SFormUtilities.populateComboBox(miClient, jcbFkEntityId, SDataConstants.CFGU_COB_ENT, new int[] { mnFkCobId, SDataConstantsSys.CFGS_CT_ENT_PLANT });
        }
    }
    ...
    public void setRegistry(erp.lib.data.SDataRegistry registry) {
        moDrcEntry = (SDataDrcEntry) registry;

        moFieldFkEntityId.setKey(mbDirectCost ?
            new int[] { moDrcEntry.getFkOrdYearId_n(), moDrcEntry.getFkOrdDocId_n() } :
            new int[] { moDrcEntry.getFkCobId_n(), moDrcEntry.getFkEntId_n() });
    }
    ...
    public erp.lib.data.SDataRegistry getRegistry() {
        ...
        if (moDrcEntry == null) {
            moDrcEntry = new SDataDrcEntry();

            moDrcEntry.setFkUserNewId(miClient.getSession().getUser().getPkUserId());

        } else {
            moDrcEntry.setFkUserEditId(miClient.getSession().getUser().getPkUserId());
        }

        if (mbDirectCost) {
            moDrcEntry.setDbmsProductionOrder(jcbFkEntityId.getSelectedItem().toString());
            moDrcEntry.setFkOrdYearId_n(moFieldFkEntityId.getKeyAsIntArray()[0]);
            moDrcEntry.setFkOrdDocId_n(moFieldFkEntityId.getKeyAsIntArray()[1]);
        }
        else {
            moDrcEntry.setDbmsPlant(jcbFkEntityId.getSelectedItem().toString());
            moDrcEntry.setFkCobId_n(moFieldFkEntityId.getKeyAsIntArray()[0]);
            moDrcEntry.setFkEntId_n(moFieldFkEntityId.getKeyAsIntArray()[1]);
        }
        return moDrcEntry;
    }
    ...
    public void setValue(int type, java.lang.Object value) {
        switch (type) {
            case 1:
                mnFkCobId = (Integer) value;
                break;
            case 2:
                mbDirectCost = (Boolean) value;

                if (mbDirectCost) {
                    this.setTitle("Horas por día - Orden de producción");
                    jlFkEntityId.setText("Orden producción:");
                }
                else {
                    this.setTitle("Horas por día - Planta");
