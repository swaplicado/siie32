 ...
 */
public class SViewUserRightRole extends erp.lib.table.STableTab {

    private erp.lib.table.STabFilterDeleted moTabFilterDeleted;

    public SViewUserRightRole(erp.client.SClientInterface client, java.lang.String tabTitle) {
        super(client, tabTitle, SDataConstants.USRX_RIGHT_ROL);
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        aoTableColumns[i++] = new STableColumn(SLibConstants.DATA_TYPE_STRING, "tp_rol", "Tipo rol", 150);
        aoTableColumns[i++] = new STableColumn(SLibConstants.DATA_TYPE_STRING, "rol", "Rol", 150);
    ...
    public void createSqlQuery() {
        ...
        msSql = "SELECT pu.id_usr, pu.id_rol, u.usr, u.b_del, p.rol, pt.tp_rol, pl.tp_lev, " +
                "0 AS f_id_co, '' AS f_co, 0 AS f_co_b_del, 0 AS f_id_bpb, '' AS f_bpb, 0 AS f_bpb_b_del " +
                "FROM erp.usru_rol_usr AS pu " +
                "INNER JOIN erp.usru_usr AS u ON " +
                "pu.id_usr = u.id_usr " +
                "INNER JOIN erp.usrs_rol AS p ON " +
                "pu.id_rol = p.id_rol " +
                "INNER JOIN erp.usrs_tp_rol AS pt ON " +
                "p.fid_tp_rol = pt.id_tp_rol " +
                "INNER JOIN erp.usrs_tp_lev AS pl ON " +
                "pu.fid_tp_lev = pl.id_tp_lev " +
                (sqlWhere.length() == 0 ? "" : "WHERE " + sqlWhere) +
                "UNION " +
                "SELECT pu.id_usr, pu.id_rol, u.usr, u.b_del, p.rol, pt.tp_rol, pl.tp_lev, " +
                "c.id_co AS f_id_co, c.co AS f_co, c.b_del AS f_co_b_del, 0 AS f_id_bpb, '' AS f_bpb, 0 AS f_bpb_b_del " +
                "FROM erp.usru_rol_co AS pu " +
                "INNER JOIN erp.usru_usr AS u ON " +
                "pu.id_usr = u.id_usr " +
                ...
                "ORDER BY usr, id_usr, f_co, f_id_co, id_rol ";
    }
}
