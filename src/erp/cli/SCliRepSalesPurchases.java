...
public class SCliRepSalesPurchases {
    ...
    public static void main(String[] args) {
        try {
            ...
            File report = createPdfReport(dbErp, new ArrayList<>(Arrays.asList(SLibUtilities.textExplode(companiesIds, ";")))); 
    ...
    private static File createPdfReport(SDbDatabase db, ArrayList<String> ids) throws Exception {
        HashMap<String, Object> map = new HashMap<>();
        JasperPrint jasperPrint;
        String sqlParam = "";
        String sqlWhereParam = "";
        String sql;
        String sqlWhere = "";
        ...
        for (String id : ids) {
            sqlWhere += (sqlWhere.isEmpty() ? "WHERE " : "OR ") + "id_co = " + id + " ";
            sqlWhereParam += (sqlWhereParam.isEmpty() ? "WHERE " : "AND ") + "id_bp <> " + id + " ";
            sql = "SELECT co_key FROM erp.cfgu_co WHERE id_co = " + id;
            resultSet = db.getConnection().createStatement().executeQuery(sql);
            if (resultSet.next()) {
                companiesNames += (companiesNames.isEmpty() ? "" : " + ") + resultSet.getString("co_key") + "";
            }
        }
        ...
        while (resultSet.next()) {
            sqlParam += (sqlParam.isEmpty() ? "" : "UNION ") +
                    ...
                    "(SELECT SUM(cfd.fid_st_xml) FROM " + resultSet.getString("bd") + ".trn_cfd AS cfd WHERE cfd.fid_dps_year_n = d.id_year AND cfd.fid_dps_doc_n = d.id_doc AND COALESCE(cfd.fid_st_xml, 1) = 1 ) AS _st_xml " +
                    "FROM " + resultSet.getString("bd") + ".trn_dps_ety AS de " +
                    "INNER JOIN " + resultSet.getString("bd") + ".trn_dps AS d ON de.id_year = d.id_year AND de.id_doc = d.id_doc " +
                    ...
                    "AND d.dt BETWEEN '" + SLibUtils.DbmsDateFormatDate.format(resultSet.getDate("ini")) + "' " +
                    "AND '" + SLibUtils.DbmsDateFormatDate.format(resultSet.getDate("fin")) + "' " +
                    "GROUP BY item, bp " +
                    "ORDER BY item, bp) " +
                    "UNION " +
                    ...
                    "FROM " + resultSet.getString("bd") + ".trn_dps_ety AS de " +
                    "INNER JOIN " + resultSet.getString("bd") + ".trn_dps AS d ON de.id_year = d.id_year AND de.id_doc = d.id_doc " +
                    "INNER JOIN " + resultSet.getString("bd") + ".trn_dps_dps_adj AS adj ON de.id_year = adj.id_adj_year AND de.id_doc = adj.id_adj_doc AND de.id_ety = adj.id_adj_ety " +
                    "INNER JOIN " + resultSet.getString("bd") + ".trn_dps_ety AS ded ON adj.id_dps_year = ded.id_year AND adj.id_dps_doc = ded.id_doc AND adj.id_dps_ety = ded.id_ety " +
                    "INNER JOIN erp.bpsu_bp AS b ON d.fid_bp_r = b.id_bp " +
                    "INNER JOIN erp.bpsu_bp_ct AS bct ON b.id_bp = bct.id_bp AND bct.id_ct_bp = 3 " +
                    "INNER JOIN erp.bpsu_tp_bp AS btp ON bct.fid_tp_bp = btp.id_tp_bp AND btp.id_ct_bp = 3 " +
        ...
        map.put("sCompanyName", companiesNames.toUpperCase());
        ...
        map.put("sSqlWhereParam", sqlWhereParam);
