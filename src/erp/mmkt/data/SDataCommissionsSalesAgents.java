 ...
 */
public class SDataCommissionsSalesAgents extends erp.lib.data.SDataRegistry implements java.io.Serializable {
    ...
    protected java.util.Vector<erp.mmkt.data.SDataCommissionsSalesAgent> mvDbmsCommissionsSalesAgent;

    public SDataCommissionsSalesAgents() {
        super(SDataConstants.MKTX_COMMS_SAL_AGTS);
        mvDbmsCommissionsSalesAgent = new Vector<SDataCommissionsSalesAgent>();
    ...
    public double getSupervisorPercentage() { return mdAgentPercentage; }
    ...
    public java.util.Vector<erp.mmkt.data.SDataCommissionsSalesAgent> getDbmsCommisionsSalesAgent() { return mvDbmsCommissionsSalesAgent; }
    ...
    public java.lang.Object getPrimaryKey() {
        return new Object[] { mnPkSalesAgentId, mnPkLinkTypeId, mnPkReferenceId, mtPkDateStartId };
    }
    ...
    public int read(java.lang.Object pk, java.sql.Statement statement) {
        ...
        SDataCommissionsSalesAgent oCommisionsSalesAgent = null;
        ...
        try {
            ...
            if (!resultSet.next()) {
                ...
                mnPkSalesAgentId = resultSet.getInt("cm.id_sal_agt");
                ...
                while (resultSet.next()) {
                    oCommisionsSalesAgent = new SDataCommissionsSalesAgent();
                    if (oCommisionsSalesAgent.read(new Object[] { resultSet.getInt("id_sal_agt"), resultSet.getInt("id_tp_link"), resultSet.getInt("id_ref"), resultSet.getDate("id_dt_start").toString() }, statementAux) != SLibConstants.DB_ACTION_READ_OK) {
                        throw new Exception(SLibConstants.MSG_ERR_DB_REG_READ_DEP);
                    }
                    else {
                        mvDbmsCommissionsSalesAgent.add(oCommisionsSalesAgent);
                    }
                }
    ...
    public int save(java.sql.Connection connection) {
        ...
        try {
            for (int i = 0; i < mvDbmsCommissionsSalesAgent.size(); i++) {
                nParam = 1;
                oCommisionsSalesAgent = mvDbmsCommissionsSalesAgent.get(i);

                callableStatement = connection.prepareCall(
                        "{ CALL mkt_comms_sal_agt_save(" +
                        "?, ?, ?, ?, ?, " +
                        "?, ?, ?, ?, ?, " +
                        "?, ?, ?, ?, ?) }");
                callableStatement.setInt(nParam++, oCommisionsSalesAgent.getPkSalesAgentId());
                callableStatement.setInt(nParam++, oCommisionsSalesAgent.getPkLinkTypeId());
                callableStatement.setInt(nParam++, oCommisionsSalesAgent.getPkReferenceId());
                callableStatement.setDate(nParam++, new java.sql.Date(oCommisionsSalesAgent.getPkDateStartId().getTime()));
                callableStatement.setDouble(nParam++, oCommisionsSalesAgent.getAgentPercentage());
                callableStatement.setDouble(nParam++, oCommisionsSalesAgent.getAgentValueUnitary());
                callableStatement.setDouble(nParam++, oCommisionsSalesAgent.getAgentValue());
                callableStatement.setDouble(nParam++, oCommisionsSalesAgent.getSupervisorPercentage());
                callableStatement.setDouble(nParam++, oCommisionsSalesAgent.getSupervisorValueUnitary());
                callableStatement.setDouble(nParam++, oCommisionsSalesAgent.getSupervisorValue());
                callableStatement.setBoolean(nParam++, oCommisionsSalesAgent.getIsDeleted());
                callableStatement.setInt(nParam++, oCommisionsSalesAgent.getFkCommissionsTypeId());
                callableStatement.setInt(nParam++, mbIsRegistryNew ? oCommisionsSalesAgent.getFkUserNewId() : oCommisionsSalesAgent.getFkUserEditId());
