 ...
 */
public class SViewPriceList extends erp.lib.table.STableTab implements java.awt.event.ActionListener {

    private erp.lib.table.STabFilterDeleted moTabFilterDeleted;

    public SViewPriceList(erp.client.SClientInterface client, java.lang.String tabTitle, int auxType01) {
        super(client, tabTitle, SDataConstants.MKT_PLIST, auxType01);
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        aoTableColumns[i++] = new STableColumn(SLibConstants.DATA_TYPE_STRING, "tp.tp_plist", "Tipo lista precios", 150);
        aoTableColumns[i++] = new STableColumn(SLibConstants.DATA_TYPE_STRING, "pr.tp_price_upd", "Tipo actualización", 100);
        ...
        mvSuscriptors.add(SDataConstants.MKT_PLIST_GRP);
        mvSuscriptors.add(SDataConstants.MKT_PLIST_ITEM);
    ...
    public void createSqlQuery() {
        ...
        msSql = "SELECT p.id_plist, p.plist, p.plist_num, p.per, p.qty_min, p.b_del, " +
            "tp.tp_plist, pr.tp_price_upd, grp.plist_grp, c.cur_key, " +
            "p.ts_new, p.ts_edit, p.ts_del, un.usr, ue.usr, ud.usr " +
            "FROM mkt_plist AS p " +
            "INNER JOIN erp.mkts_tp_plist AS tp ON " +
            "p.fid_tp_plist = tp.id_tp_plist " +
            "INNER JOIN erp.mkts_tp_price_upd AS pr ON " +
            "p.fid_tp_price_upd = pr.id_tp_price_upd " +
            "INNER JOIN erp.trns_ct_dps AS ct ON " +
            "p.fid_ct_dps = ct.id_ct_dps " +
            "INNER JOIN erp.cfgu_cur AS c ON " +
            "p.fid_cur = c.id_cur " +
            "INNER JOIN erp.usru_usr AS un ON " +
            "p.fid_usr_new = un.id_usr " +
            "INNER JOIN erp.usru_usr AS ue ON " +
            "p.fid_usr_edit = ue.id_usr " +
            "INNER JOIN erp.usru_usr AS ud ON " +
            "p.fid_usr_del = ud.id_usr " +
            "LEFT JOIN mkt_plist_grp AS grp ON " +
            "p.fid_plist_grp_n = grp.id_plist_grp " +
