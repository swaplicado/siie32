 ...
 */
public class SViewDistributionChannel extends erp.lib.table.STableTab implements java.awt.event.ActionListener {

    private erp.lib.table.STabFilterDeleted moTabFilterDeleted;

    public SViewDistributionChannel(erp.client.SClientInterface client, java.lang.String tabTitle) {
        super(client, tabTitle, SDataConstants.MKTU_DIST_CHAN);
        initComponents();
    }
    ...
    private void initComponents() {
        ...
        i = 0;
        aoKeyFields[i++] = new STableField(SLibConstants.DATA_TYPE_INTEGER, "d.id_dist_chan");
        for (i = 0; i < aoKeyFields.length; i++) {
            moTablePane.getPrimaryKeyFields().add(aoKeyFields[i]);
        }

        i = 0;
        aoTableColumns[i++] = new STableColumn(SLibConstants.DATA_TYPE_STRING, "d.dist_chan", "Canal distribución", 300);
        aoTableColumns[i++] = new STableColumn(SLibConstants.DATA_TYPE_BOOLEAN, "d.b_can_edit", "Modificable", STableConstants.WIDTH_BOOLEAN_2X);
        aoTableColumns[i++] = new STableColumn(SLibConstants.DATA_TYPE_BOOLEAN, "d.b_can_del", "Eliminable", STableConstants.WIDTH_BOOLEAN_2X);
    ...
    public void createSqlQuery() {
        ...
        msSql = "SELECT d.*, d.b_can_edit AS " + STableConstants.FIELD_IS_EDITABLE + ", un.usr, ue.usr, ud.usr " +
                "FROM mktu_dist_chan AS d " +
