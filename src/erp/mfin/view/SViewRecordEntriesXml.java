...
public class SViewRecordEntriesXml extends erp.lib.table.STableTab implements java.awt.event.ActionListener {
    ...
    public void createSqlQuery() {
        ...
        msSql = "SELECT * FROM " +
                ...
                "get_data_xml('<cfdi:Emisor', '</cfdi:Emisor>', 'nombre=', cfd.doc_xml) AS _xml_emisor_nombre, " +
                "get_data_xml('<cfdi:Emisor', '</cfdi:Emisor>', 'rfc=', cfd.doc_xml) AS _xml_emisor_rfc, " +
                "get_data_xml('<cfdi:Receptor', '</cfdi:Receptor>', 'nombre=', cfd.doc_xml) AS _xml_receptor_nombre, " +
                "get_data_xml('<cfdi:Receptor', '</cfdi:Receptor>', 'rfc=', cfd.doc_xml) AS _xml_receptor_rfc, " +
                "get_data_xml('<cfdi:Comprobante', '</cfdi:Comprobante>', 'total=', cfd.doc_xml) AS _xml_total, " +
                "CAST(REPLACE(get_data_xml('cfdi:Complemento', '</cfdi:Complemento>', 'FechaTimbrado=', cfd.doc_xml), 'T', ' ') AS DATETIME) AS _xml_fecha_timbrado, " +
                "get_data_xml('<cfdi:Complemento', '</cfdi:Complemento>', 'UUID=', cfd.doc_xml) AS _xml_uuid " +
                "FROM trn_cfd AS cfd " +
                ...
                "AND get_data_xml('<cfdi:Emisor', '</cfdi:Emisor>', 'rfc=',cfd.doc_xml) NOT LIKE '" + miClient.getSessionXXX().getCompany().getDbmsDataCompany().getFiscalId() + "' " :
                "AND get_data_xml('<cfdi:Emisor', '</cfdi:Emisor>', 'rfc=',cfd.doc_xml) LIKE '" + miClient.getSessionXXX().getCompany().getDbmsDataCompany().getFiscalId() + "' ") +
                "UNION " +
                "SELECT re.id_year AS id_year, re.id_per AS id_per, re.id_bkc AS id_bkc, re.id_tp_rec AS id_tp_rec, re.id_num AS id_num, re.id_ety AS id_ety, cfd.id_cfd AS id_cfd, r.dt AS dt, re.concept AS concept, re.b_del AS b_del, " +
                "re.ts_new AS ts_new, re.ts_edit AS ts_edit, re.ts_del AS ts_del, acc.id_acc AS f_acc, acc.acc AS f_acc_name, cur.cur_key AS f_cur, bkc.code AS code_bkc, cob.code AS code_cob, e.ent AS ent, un.usr AS usr_un, ue.usr AS usr_ue, " +
                "ud.usr AS usr_ud, CONCAT(re.id_year, '-', erp.lib_fix_int(re.id_per, 2)) as f_per, CONCAT(re.id_tp_rec, '-', erp.lib_fix_int(re.id_num, 6)) as f_num, re.exc_rate AS exc_rate, re.debit AS debit, " +
